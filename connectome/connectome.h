...
void neuronToString(neuron n) {           //function to convert a neuron to a string in the data file
    ...
    for (int i = 0; i < n.inputsLen; i++) {     //iterate over number of inputs/weights
            ...
            //note: the issue is with how the function to_string() is dealing with the n.inputs variable
        ...
        if (inputsStr == "") {              //if empty then just assign a value
             inputsStr = to_string(n.inputs[i]);     //assign value of an input as a string
            weightsStr = to_string(n.weights[i]);   //assign value of a weight as a string
        } else {                                    //otherwise
            inputsStr.append(',' + to_string(n.inputs[i]));   //append the values if already has data in string
            weightsStr.append(',' + to_string(n.inputs[i]));
        }
    }
...
neuron stringToNeuron(int id) {     //function to read the file for a neuron
    matrixFile.open("cellularMatrixData.txt");
    ...
    */
    ...
    for (int j = 0; j <= id; j++) {
    //read out from the network into this data
        while (!data.compare(", ") && !matrixFile.eof()) {
            //read out line by line into string variables
            ...
            for (int i = 0; i < stoi(inputsLenStr); i++) {
                getline(matrixFile, data, '\n');
                inputsStr[i] = data;
            }

            for (int i = 0; i < stoi(weightsLenStr); i++) {
                getline(matrixFile, data, '\n');
                weightsStr[i] = data;
            }
        ...
        if (j == id) {
            n.cellID = stoi(cellIDStr);
            n.threshold = stoi(thresholdStr);
            n.cellOutput = stoi(cellOutputStr);

            n.inputsLen = stoi(inputsLenStr);
            n.weightsLen = stoi(weightsLenStr);

            //iterate over the number of inputs using inputsLen
            //each line is a new input val at the iterative var

            for (int i = 0; i < stoi(inputsLenStr); i++) {
                n.inputs[i] = stoi(inputsStr[i]);
            }

            for (int i = 0; i < stoi(weightsLenStr); i++) {
                n.weights[i] = stoi(weightsStr[i]);
            }
    ...
    neuron errorNeuron;
    errorNeuron.cellID = 0;
    errorNeuron.cellOutput = 0;
    errorNeuron.inputs[0] = {};
    errorNeuron.inputsLen = 1;
    errorNeuron.threshold = 0;
    errorNeuron.weights[0] = {};
    errorNeuron.weightsLen = 0;
